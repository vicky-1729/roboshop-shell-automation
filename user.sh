...
# Download user zip file
echo -e "${y}Downloading the user zip file...${s}"
curl -L -o /tmp/user.zip https://roboshop-artifacts.s3.amazonaws.com/user-v3.zip &>> "$LOG_FILES"
...
echo -e "${y}Unzipping the user zip file...${s}"
unzip /tmp/user.zip &>> "$LOG_FILES"
...
echo -e "${y}Creating service for user...${s}"
cp $script_dir/services/user.service /etc/systemd/system/user.service
validate $? "user services creation is"


# Enable and start user service
echo -e "${y}Starting the user service...${s}"
systemctl daemon-reload
systemctl enable user &>> "$LOG_FILES"
systemctl start user &>> "$LOG_FILES"
validate $? "user service start"

# Final message
echo -e "${m} user part completed successfully..!"